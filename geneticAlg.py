# This program was modified to reproduce the genetic algorithm used by khamassi et al. (https://www.semanticscholar.org/paper/A-GA-LR-wrapper-approach-for-feature-selection-in-Khammassi-Krichen/9b5c09c49d65f7a0a088408432fefa5cb9262858)
# 
# Special thanks to Manuel Calzolari for this library
# 
...
# Define we want to maximize the fitness of our evaluation function
...
def _eaFunction(population, toolbox, cxpb, mutpb, ngen, ngen_no_change=None, stats=None,
    ...
    # Evaluate the individuals with an invalid fitness (individuals that got mutated or went through crossover)
    invalid_ind = [ind for ind in population if not ind.fitness.valid]

    # Apply evaluation function on invalid individuals
    ...
    for gen in range(1, ngen + 1):
        ...
        # Vary the pool of individuals by applying crossover and mutation
        ...
        # If all chromosomes are the same, stop the optimization
        if all(x == population[0] for x in population):
            print("All chromosomes identical, stopping")
            break
...
def _mutate(individual):
    maxmutations = len(individual)//10
    nbmutations = np.random.randint(0,maxmutations)
...
def _evalFunction(individual, estimator: LogisticRegression(), X, y, X_test, y_test, groups, cv, scorer, fit_params, max_features, alpha,
                  caching, scores_cache={}):
    
    X_selected = X[:, np.array(individual, dtype=np.bool)]
    X_test_selected = X_test[:, np.array(individual, dtype=np.bool)]
    if X_selected.shape[1] == 0:
        return 0,
    estimator.fit(X_selected, y)
    accuracy_score = estimator.score(X_test_selected, y_test)
    print("Accuracy: ", accuracy_score)
    return (alpha * accuracy_score) + (1 - alpha) * (1/individual.count(1)),


class GeneticSelection(BaseEstimator, MetaEstimatorMixin, SelectorMixin):
    ...
    estimator_weight : float, default=0.2
        Weight given to the accuracy resulting from the estimator
    ...
    """
    def __init__(self, estimator: LogisticRegression, cv=None, scoring=None, fit_params=None, max_features=None,
                 verbose=0, n_jobs=1, n_population=300, crossover_proba=0.5, mutation_proba=0.2,
                 n_generations=40, n_gen_no_change=None, estimator_weight=0.2,
                 caching=False):
        ...
        self.scores_cache = {}
        self.estimator_weight = estimator_weight

    @property
    def _estimator_type(self):
        return self.estimator._estimator_type

    def fit(self, X, y, X_test, y_test, groups=None):
        ...
        """
        return self._fit(X, y, X_test=X_test, y_test=y_test, groups=groups)

    def _fit(self, X, y, X_test, y_test, groups=None):
        ...
        toolbox.register("evaluate", _evalFunction, estimator=estimator, X=X, y=y,X_test=X_test, y_test=y_test,
                         groups=groups, cv=cv, scorer=scorer, fit_params=self.fit_params,
                         max_features=max_features, caching=self.caching,
                         scores_cache=self.scores_cache, alpha=self.estimator_weight)
        ...
        self.generation_scores_ = np.array([score for score in log.select("max")])
